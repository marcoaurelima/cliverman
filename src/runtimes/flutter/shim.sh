...
make_shim() {
 ...
 # Function to create the flutter shim and default pre-installed binaries
make_shim_default() {
    local bin_path="${1}"
    local template
    template=$(< "${CLIVERMAN_RUNTIMES_PATH}/flutter/template/shim-default.template.sh")
